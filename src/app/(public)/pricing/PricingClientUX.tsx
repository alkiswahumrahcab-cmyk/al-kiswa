        ...
        <div className="relative">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start relative pb-24 md:pb-0">
                ...
                <div className="lg:col-span-8 space-y-6">
                    <div className="flex justify-between items-center text-ink-muted text-sm border-b border-border pb-2 mb-4">
                                    ...
                                    >
                                        ...
                                        <div className="p-5 md:p-6 flex flex-col md:flex-row justify-between gap-4 md:items-center relative">
                                            ...
                                            <div className="flex-1">
                                                <h3 className="text-xl md:text-2xl font-bold text-ink mb-2 flex items-center gap-2">
                                                    {route.origin} <ArrowRight size={18} className="text-gold" /> {route.destination}
                                                </h3>
                                                <div className="flex flex-wrap gap-3 text-xs md:text-sm text-ink-muted">
                                                    {route.distance && (
                                                        <span className="flex items-center gap-1 bg-surface px-2 py-1 rounded-md">
                                            ...
                                            <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center border-t md:border-t-0 border-border pt-4 md:pt-0">
                                                <div className="text-left md:text-right">
                                                    <p className="text-[10px] md:text-xs text-ink/50 uppercase tracking-wider mb-0.5">From</p>
                                                    <div className="flex items-baseline gap-1">
                                                        <span className="text-2xl md:text-3xl font-black text-gold">{formatSAR(lowestPrice)}</span>
                                                        <span className="text-sm font-bold text-ink-muted">SAR</span>
                                                    </div>
                                                </div>
                                        ...
                                        <button 
                                            ...
                                            className="w-full py-2 bg-surface hover:bg-white/10 border-t border-border flex items-center justify-center gap-2 text-xs font-semibold text-ink/80 uppercase tracking-wider transition-colors focus:outline-none"
                                        >
                                            {isExpanded ? (
                                            ...
                                            {isExpanded && (
                                                ...
                                                >
                                                    <div className="p-5 md:p-6 grid md:grid-cols-2 gap-6">
                                                        {/* Vehicle List */}
                                                        <div>
                                                            <h5 className="text-xs text-ink/50 uppercase font-bold mb-3">Vehicle Classes</h5>
                                                            <div className="space-y-2">
                                                                {priceRows.map((row: any, i: number) => row && (
                                                                    <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-surface border border-border hover:border-gold/30 transition-colors">
                                                                        <div>
                                                                            <span className="text-ink font-medium text-sm block">{row.label}</span>
                                                                            <span className="text-ink/50 text-[10px]">{row.pax} • {row.bags}</span>
                                                                        </div>
                                                                        <div className="text-right">
                                                                            <span className="text-ink font-bold">{formatSAR(row.priceSAR)}</span>
                                                        ...
                                                        <div>
                                                            <h5 className="text-xs text-ink/50 uppercase font-bold mb-3">Included Services</h5>
                                                            <div className="flex flex-wrap gap-2">
                                                                {features.map((f, i) => (
                        ...
                        >
                            ...
                            <div className="space-y-3">
                                {compareDrawerRoute.prices?.map((p: any) => {
                                    ...
                                    return (
                                        <div key={p.vehicleId} className="flex justify-between items-center p-4 rounded-xl bg-surface border border-border">
                                            <div>
                                                <div className="font-bold text-ink text-lg flex items-center gap-2">
                                                    {display.label}
                                                ...
                                                <div className="text-xs text-ink-muted mt-1 flex gap-2">
                                                    <span className="flex items-center gap-1"><Users size={12} className="text-ink/50" /> {display.pax}</span>
                                                    <span className="flex items-center gap-1"><Luggage size={12} className="text-ink/50" /> {display.bags}</span>
                                                </div>
                                            </div>
                                            <div className="text-right flex flex-col items-end">
                                                <div className="flex items-baseline gap-1">
                                                    <span className="text-xl font-black text-gold">{formatSAR(p.price)}</span>
                                                    <span className="text-xs font-bold text-ink-muted">SAR</span>
                                                </div>
                                                <Link 
