        ...
        {isOpen && (
          <div id="mobile-drawer-portal" className="fixed inset-0 z-[9999] flex flex-col justify-end pointer-events-auto">
            ...
            <motion.div 
              ...
              className="relative bg-surface w-full rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.1)] border-t border-border overflow-hidden flex flex-col max-h-[90vh]"
            >
              <div className="p-4 border-b border-border flex flex-col items-center sticky top-0 z-10 bg-surface">
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0, y: -5 }}
          className="absolute left-0 right-0 z-50 bg-surface border border-border rounded-xl shadow-xl overflow-hidden mt-2"
          style={{ top: '100%' }}
        >
          {children}
        </motion.div>
