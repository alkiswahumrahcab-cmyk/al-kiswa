...
export default function ImpactStats({ lang = 'en' }: Props) {
  ...
  return (
    <section
      ...
      className="py-20 md:py-24 relative overflow-hidden bg-surface-alt border-y border-border backdrop-blur-sm"
    >
      ...
      <div className="max-w-6xl mx-auto px-4 relative z-10">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
          {COMPANY_STATS.map((stat, index) => (
            <GlassCard
              key={stat.id}
              delay={index * 0.1}
              className="flex flex-col items-center justify-center text-center p-8 group hover:border-gold-soft transition-all duration-500 bg-surface border border-border hover:shadow-gold h-full"
            >
              <div 
              ...
              <div 
                className="text-4xl md:text-5xl font-semibold text-ink mb-2 font-display tracking-tight"
                role="img"
              ...
              {lang === 'ar' ? (
                <>
                  <p className="text-ink-muted text-sm font-medium uppercase tracking-[0.1em] mb-1" style={{ fontFamily: 'var(--font-tajawal)' }}>
                    {stat.labelAr}
                  </p>
                  <p className="text-ink-muted text-xs mb-3 font-sans uppercase tracking-widest">
                    {stat.label}
                  </p>
                  <p className="text-ink-muted text-xs mb-4 opacity-0 h-0 overflow-hidden group-hover:h-auto group-hover:opacity-100 transition-all duration-300" style={{ fontFamily: 'var(--font-tajawal)' }}>
                    {stat.descriptionAr || stat.description}
                  </p>
                </>
              ) : (
                <>
                  <p className="text-ink-muted text-sm font-medium uppercase tracking-[0.1em] mb-1">
                    {stat.label}
                  </p>
                  <p
                    className="text-ink-muted text-xs mb-3"
                    dir="rtl"
                    lang="ar"
                  ...
                  <p className="text-ink-muted text-xs mb-4 opacity-0 h-0 overflow-hidden group-hover:h-auto group-hover:opacity-100 transition-all duration-300">
                    {stat.description}
                  </p>
                </>
              )}
              ...
              <Link
                ...
                className="mt-auto text-xs text-gold-strong hover:text-gold border-b border-gold-soft hover:border-gold transition-colors pb-0.5"
        ...
        <p className="text-center text-ink-muted text-sm mt-8 italic" style={{ fontFamily: 'var(--font-tajawal)' }}>
