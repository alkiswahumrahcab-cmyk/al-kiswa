    ...
    return (
        <div className="flex flex-col h-full w-96 bg-transparent z-20">
                    ...
                    return (
                        <motion.div
                            ...
                            className={`
                                ...
                                ${isActive
                                    ? 'bg-gold/10 border-gold/50 shadow-[0_0_15px_-5px_hsl(var(--gold-glow) / 0.3)]'
                            ...
                            <div className="flex justify-between items-start">
                                <div>
                                    <div className="flex items-center gap-2 mb-1">
                                        <span className={`w-1.5 h-1.5 rounded-full ${isActive ? 'bg-gold animate-pulse' : 'bg-green-500'}`} />
                                        <span className="text-[10px] font-bold text-n-400 uppercase tracking-wider">Available</span>
                                    </div>
                                    <h3 className={`font-bold text-lg mb-1 transition-colors ${isActive ? 'text-white' : 'text-n-300 group-hover:text-white'}`}>
                                        {route.destination}
                                    </h3>
                                    <p className="text-xs text-n-500 flex items-center gap-2">
                                        <Clock size={12} className={isActive ? 'text-gold' : ''} /> {route.duration} • {route.distance}
                                    </p>
                                </div>
                                <div className={`
                                    w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300
                                    ${isActive ? 'bg-gold text-black rotate-90' : 'bg-white/10 text-n-400 group-hover:bg-white/20 group-hover:text-white'}
                                `}>
            ...
            <div className="p-4 bg-white/5 border-t border-white/5">
                <div className="flex items-center gap-3 text-xs text-n-400 font-medium">
