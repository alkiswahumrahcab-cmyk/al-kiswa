...
const PricingCard = ({ route, dbVehicleId }: { route: RouteProduct; dbVehicleId: string }) => {
    ...
    return (
        <GlassCard className="group h-full flex flex-col justify-between p-4 rounded-2xl bg-neutral-900/50 border border-white/5 hover:border-gold/30 transition-all duration-300 hover:shadow-[0_0_20px_hsl(var(--gold-glow) / 0.1)] hover:-translate-y-1 relative overflow-hidden">
            {/* Decorator */}
            <div className="absolute top-0 right-0 w-16 h-16 bg-gradient-to-bl from-gold/10 to-transparent rounded-bl-full -mr-6 -mt-6 transition-transform group-hover:scale-150 duration-500 ease-out pointer-events-none" />
            ...
            <div>
                ...
                <div className="text-center space-y-2 mb-4">
                    ...
                    <div className="text-lg sm:text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-gold via-[#F3D383] to-gold font-sans tracking-tight">
                        {route.price}
                    </div>
                </div>
            </div>
            ...
            <div className="space-y-3 pt-3 border-t border-white/10">
                ...
                <Link
                    ...
                    className="block w-full bg-gradient-to-r from-gold to-gold-dark text-black font-bold py-2.5 px-4 rounded-xl text-xs sm:text-sm transition-all duration-300 shadow-lg hover:shadow-[#D4AF37]/30 text-center uppercase tracking-wider hover:scale-[1.02]"
                >
                    Book Now
                </Link>
            </div>
