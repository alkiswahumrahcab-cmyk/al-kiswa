    ...
    return (
        <AnimatedSection className="pt-10 md:pt-16 pb-20 md:pb-28 relative overflow-hidden bg-bg">
            ...
            <div className="container px-4">
                <FadeIn>
                    <h2 className={`text-4xl md:text-5xl font-semibold text-center mb-6 text-ink leading-tight ${lang === 'ar' ? 'font-ar-head' : 'font-display'}`}>
                        {t.heading}
                    </h2>
                    <p className={`text-lg text-ink-muted text-center max-w-2xl mx-auto mb-16 font-light leading-relaxed ${lang === 'ar' ? 'font-ar-body' : ''}`}>
                        {t.sub}
                    </p>
                </FadeIn>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {t.features.map((feature, index) => (
                        <div key={index} className="card text-center group h-full p-8 flex flex-col items-center justify-start">
                            ...
                            <h3 className={`text-2xl font-semibold mb-2 text-ink group-hover:text-gold transition-colors ${lang === 'ar' ? 'font-ar-head' : 'font-display'}`}>{feature.title}</h3>
                            <p className={`text-gold-deep font-semibold mb-4 text-sm tracking-widest uppercase ${lang === 'ar' ? 'font-ar-body tracking-normal' : ''}`}>{feature.arabic}</p>
                            <p className={`text-ink-muted leading-relaxed group-hover:text-ink transition-colors ${lang === 'ar' ? 'font-ar-body' : ''}`}>{feature.desc}</p>
                        </div>
                    ))}
                </div>
            </div>
